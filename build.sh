...
########################################################
# 
#            Vampire Theme Installation
#
#   Created and maintained by Pterodactyl Resources
#
#            https://pteroresources.rf.gd
#
########################################################
...
curl --silent \
  ...
  https://github.com/Yeliactyl/VampireClient/releases/latest |
  grep '"tag_name":' |
  sed -E 's/.*"([^"]+)".*/\1/'
}
...
SUPPORT_LINK="https://discord.gg/bMNduNcxWQ"
...
if [ "$PANEL_VERSION" == "1.7.0" ] || [ "$PANEL_VERSION" == "1.8.1" ]; then
    ...
    print_error "Incompatible Version, please use pterodactyl 1.7.0 or 1.8.1!"
    exit 1
fi
}
...
if [ -d "$PTERO/PanelBackup[VampireTheme]" ]; then
    print "There is already a backup, skipping step."
  else
    cd $PTERO
    if [ -d "$PTERO/node_modules" ]; then
        tar -czvf "PanelBackup[VampireTheme].tar.gz" --exclude "node_modules" -- * .env
        mkdir -p "$PTERO/PanelBackup[VampireTheme]"
        mv "$PTERO/PanelBackup[VampireTheme].tar.gz" "$PTERO/PanelBackup[VampireTheme]"
      else
        tar -czvf "PanelBackup[VampireTheme].tar.gz" -- * .env
        mkdir -p "$PTERO/PanelBackup[VampireTheme]"
        mv "$PTERO/PanelBackup[VampireTheme].tar.gz" "$PTERO/PanelBackup[VampireTheme]"
    fi
fi
...
mkdir -p $PTERO/temp
curl -sSLo $PTERO/temp/VampireTheme.tar.gz https://raw.githubusercontent.com/Yeliactyl/VampireClient/"${SCRIPT_VERSION}"/VampireTheme.tar.gz
tar -xzvf $PTERO/temp/VampireTheme.tar.gz -C $PTERO/temp
cp -rf -- $PTERO/temp/VampireTheme/* $PTERO
rm -rf $PTERO/temp
}
...
print_brake 50
echo
echo -e "${GREEN}* The theme was successfully installed."
